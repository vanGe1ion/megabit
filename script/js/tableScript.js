    ...
    $.ajax({
        ...
        success: function (result) {
            if(result)
                ...
                ThrowNotice("#notices", "Warning", "Внимание!", "sql", " Не возможно удалить запись. Возможно она связана с другими таблицами");
        },
        beforeSend: function (){
            return $("#confirm").dialog("open");
        },
        error: function () {
            ThrowNotice("#notices", "Error", "Ошибка!", "ajax","Ошибка удаления записи");
        }
    });
};
        ...
        $.ajax({
            ...
            success: function (result) {
                if(result)
                    ...
                    ThrowNotice("#notices", "Warning", "Внимание!", "sql", " Не возможно сохранить изменения. Возможно поле с таким именем уже существует");
            },
            error: function () {
                ThrowNotice("#notices", "Error", "Ошибка!", "ajax", "Ошибка редактирования записи");
            }
        })
    })
};
...
var addHandler = function (buttonHolder, tableData = null) {
    buttonHolder.parent().parent().before("<tr class='adder'></tr>");
    ...
    adderRow.children().children(".cancel").bind("click",function () {
        ...
        currow.children().children(":input").hide(150, function () {currow.html(""); $(".adder").remove()});
        ...
        $.ajax({
            ...
            success: function (result) {
                if(result) {
                    ...
                    ThrowNotice("#notices", "Warning", "Внимание!", "sql", " Не возможно добавить запись. Возможно она уже существует");
            },
            error: function () {
                ThrowNotice("#notices", "Error", "Ошибка!", "ajax", "Ошибка добавления записи");
            }
        });
    });
};
